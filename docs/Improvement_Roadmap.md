...
|---|---|---|
| 1. `store_corrected=False` | Removes the previous correction from the next surface calculation. With small measured error and $\lambda>1$, corrected memory can sustain alternating corrections of magnitude $k$. | Measured errors can still oscillate, and useful temporal behavior might be lost. |
